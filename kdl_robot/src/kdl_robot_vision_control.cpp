...
// Global variables
std::vector<double> jnt_pos(7,0.0), init_jnt_pos(7,0.0), jnt_vel(7,0.0), aruco_pose(7,0.0);   //vettori di 7 elementi tutti inizializzati a 0
...
//callback per la ricezione di messaggi riguardo stato dei giunti e posa del marker
void jointStateCallback(const sensor_msgs::JointState & msg)
{
    robot_state_available = true;

    // Update joints (svuota i vettori per prepararli a contenere altro)
    jnt_pos.clear();
    jnt_vel.clear();

/*msg.position[i] rappresenta il valore della posizione della giuntura al numero di indice i all'interno del messaggio JointState.
msg.velocity[i] rappresenta il valore della velocità della giuntura al numero di indice i all'interno del messaggio JointState.
Entrambi questi valori vengono aggiunti rispettivamente ai vettori jnt_pos e jnt_vel utilizzando la funzione push_back(), 
che estende dinamicamente la dimensione del vettore e aggiunge un nuovo elemento con il valore specificato. Alla fine di questo processo, 
i vettori jnt_pos e jnt_vel conterranno le informazioni aggiornate sullo stato delle giunture ricevute dal messaggio JointState.*/
...
/*Aggiunge i valori delle coordinate XYZ e dell'orientazione (quaternione) del messaggio PoseStamped al vettore aruco_pose. 
Questo processo sembra essere una copia dei dati di posizione e orientazione del marker ArUco dal messaggio PoseStamped al vettore aruco_pose.*/
...
// Specify an end-effector: camera in flange transform
    KDL::Frame ee_T_cam;        //ee
    //ee_T_cam.M = KDL::Rotation::RotY(0.0) * KDL::Rotation::RotZ(0.0);
    ...
    KDL::Frame Fi = robot.getEEFrame();     //ee
    ...
    while (true)
    {
        if (robot_state_available && aruco_pose_available)
        {
            ...
            KDL::Frame cam_T_object(KDL::Rotation::Quaternion(aruco_pose[3], aruco_pose[4], aruco_pose[5], aruco_pose[6]),
                                    KDL::Vector(aruco_pose[0], aruco_pose[1], aruco_pose[2]));      //ee
            ...
            //--------------------------2a------------------------------------
            //Calcolo Frame Desiderata
            /*
            KDL::Frame des_frame=cam_T_object;
            des_frame.p=cam_T_object.p-KDL::Vector(0,0,0.5);
            des_frame.M=cam_T_object.M*KDL::Rotation::RotX(M_PI);
            KDL::Frame base_des_frame=robot.getEEFrame()*des_frame;
            Eigen::Matrix<double, 3, 1> e_p = computeLinearError(toEigen(base_des_frame.p), toEigen(robot.getEEFrame().p));
            Eigen::Matrix<double, 3, 1> e_o = computeOrientationError(toEigen(base_des_frame.M),toEigen(robot.getEEFrame().M));  //errore
            ...
            */
            ...
            L.block(0,0,3,3) = (-1/c_Po.norm())*(Eigen::Matrix<double, 3, 3>::Identity()-s*s.transpose());
            ...
            double k=6;
            Eigen::Vector3d s_d=Eigen::Vector3d(0,0,1);
            dqd.data = k*LJ_pinv*s_d + 10*N*(qdi-toEigen(jnt_pos));    
            ...
            */
            ...
            //std::cout << "desired_pose: " << std::endl << e_orient << std::endl;
            // std::cout << "current_pose: " << std::endl << robot.getEEFrame() << std::endl;

        }
        else
        {
            dqd.data = 0.5 * KP * (toEigen(init_jnt_pos) - toEigen(jnt_pos));
        }
